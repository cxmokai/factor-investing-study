...
| 周 | 主题 | 学习内容 | 资源 |
|---|------|---------|------|
| 1 | 市场基础 | 股票、债券、基金是什么；交易机制（订单簿、撮合）；A股/美股市场结构 | 《漫步华尔街》前6章 |
...
**练习项目**：
1. 用 tushare 拉取沪深300成分股3年日频数据
2. 计算个股收益率、波动率、夏普比
3. 实现等权组合 vs 市值加权组合，对比表现
4. 用 OLS 对个股收益做 Fama-French 三因子回归，理解 alpha 和 beta
...
| 周 | 因子 | 构造方法 | 经济学直觉 |
|---|------|---------|-----------|
| 7 | **市值因子 (Size)** | 流通市值排序分组 | 小盘股风险补偿 |
| 8 | **价值因子 (Value)** | BP、EP、DP等估值指标 | 低估修复 |
| 9 | **动量因子 (Momentum)** | 过去N月累计收益率 | 行为金融：反应不足 |
| 10 | **质量因子 (Quality)** | ROE、盈利稳定性、低杠杆 | 好公司溢价 |
...
### 编码项目
构建一个**完整的多因子选股策略**：
1. 合成一个综合因子（融合 Value + Quality + Momentum）
2. 每月调仓，选综合因子排名前N的股票
3. 加入交易成本（佣金+滑点+冲击成本）
4. 完整绩效评估与风险归因
...
- **高频因子**：日内数据、订单流因子（资金需求高，了解即可）

### 实战建议
- 在 **聚宽(JoinQuant)** 或 **米筐(RiceQuant)** 平台上部署策略进行模拟盘
- 参加一次 WorldQuant Brain 或类似量化竞赛
- 开始用真实小资金跑一个简单策略，体验实盘与回测的差距
