...
你在金融知识 Day 10 已经了解了因子投资的起源。现在我们把直觉升级成精确的定义。
...
在金融知识 Day 8-10 学过的定价模型里，已经出现了这些因子。本计划 Day 4-8 会逐个深入讲解，今天先建立全景。
...
| 步骤 | 做什么 | 本计划对应 |
|------|--------|-----------|
| 1. 获取数据 | 拿到股票价格和相关指标 | Day 4 开始实操 |
| 2. 定义因子 | 选择一个特征（如 PE），算出每只股票的因子值 | Day 4-8 |
| 3. 构建因子组合 | 按因子值把股票分组 | **Day 1-2（今天开始）** |
| 4. 计算因子收益率 | 多头组收益 - 空头组收益 | **Day 1-2** |
| 5. 统计检验 | 因子收益率是否显著不为零 | Day 2-3 |
| 6. 多因子合成 | 把多个有效因子组合起来 | Day 10 |
| 7. 组合构建与回测 | 确定每只股票的权重，评估整体表现 | Day 11-14 |
...
> **本计划的路线：** 先学排序法（Day 1-2），再学回归法（Day 3）。排序法是基础，更直觉，也是论文中最常见的方法。
...
> **关键细节：** 排序用的是 6 月末的 PE（已知信息），预测的是 7 月的收益率（未来信息）。排序时**绝对不能**用 7 月的数据——这就是"前视偏差"，Day 11 会专门讲。
...
> **注意：** 实际研究中应该用 Newey-West 标准误替代普通标准误（因为月度收益率可能有自相关），t 值会有所降低。Newey-West 的原理在数学 Day 7 讲过，Day 2 会在因子检验中具体应用。
...
| 内容 | 为什么不需要 |
|------|------------|
| 6 个经典因子的具体构造方法 | Day 4-8 会逐个详细讲，今天只需知道名字和大致逻辑 |
| Fama-MacBeth 回归的具体步骤 | Day 3 专门讲 |
| GRS 检验 | Day 9 讲多因子模型时会用到 |
| 因子正交化 | Day 10 讲 |
| A 股的具体因子表现 | Day 4-8 每个因子会给 A 股实证数据 |
| 排序法的数学证明 | 不需要证明，能做就行 |
