...
$$[w_1, w_2, w_3] \cdot [r_1, r_2, r_3] = w_1 r_1 + w_2 r_2 + w_3 r_3$$

这个运算在因子投资中无处不在，因为：

> **投资组合收益：** 权重向量 $\cdot$ 收益率向量
...
**向量有几个数字，就是几维。** 记号：$n$ 维向量写作 $\mathbb{R}^n$。

| 向量 | 维度 | 说明 |
|------|------|------|
| [3, -1] | 2 维（$\mathbb{R}^2$） | 平面上的点 |
| [3, -1, 2] | 3 维（$\mathbb{R}^3$） | 空间中的点 |
| 60 个月收益率 | 60 维（$\mathbb{R}^{60}$） | 无法画图，但数学上完全一样 |
| 5000 只股票的权重 | 5000 维（$\mathbb{R}^{5000}$） | 因子投资的日常 |
...
- [x] 我知道向量就是一列有序的数字
- [x] 我能说出因子投资中 3 个向量的例子（权重、收益率、因子暴露）
- [x] 我能手算两个向量的点积
- [x] 我能计算一个 3 只股票的投资组合的收益率（用点积）
- [x] 我知道转置就是行列互换，记号是 $'$ 或 $^T$
...
这个表格就是一个 $4 \times 3$ 矩阵（4 行 3 列），记作：
...
**$m \times n$ 矩阵 = $m$ 行 $n$ 列。** 永远先说行，再说列。

| 金融场景 | 矩阵维度 | 行代表 | 列代表 |
|---------|---------|--------|--------|
| 60 个月、300 只股票的收益率 | $60 \times 300$ | 月份 | 股票 |
| 5000 只股票、3 个因子的暴露 | $5000 \times 3$ | 股票 | 因子 |
| 300 只股票的协方差矩阵 | $300 \times 300$ | 股票 | 股票 |

> 因子投资中遇到的矩阵通常很大（几千行、几百列），但运算规则和 $2 \times 2$ 矩阵完全一样。
...
- $X$ 是 $T \times K$ 矩阵（$T$ 个月，$K$ 个因子）
- $X'$ 是 $K \times T$ 矩阵
- $X'X$ 是 $K \times K$ 矩阵（维度检查：$(K \times T) \times (T \times K) = K \times K$）

> 现在不需要理解 $X'X$ 为什么出现在回归公式里（Day 17 会详细讲），只需要能做维度检查。

#### 2.5 几个特殊矩阵

| 名称 | 长什么样 | 因子投资中哪里会见到 |
|------|---------|-------------------|
| 单位矩阵 $I$ | 对角线是 1，其余是 0 | $A \times I = A$，相当于"乘以 1" |
...
> 对称矩阵意味着第 $(i,j)$ 个元素等于第 $(j,i)$ 个元素。协方差矩阵就是对称的，因为 $\text{Cov}(A,B) = \text{Cov}(B,A)$。
...
一个 $2 \times 2$ 矩阵作用在平面上的一个单位正方形，会把它变成一个平行四边形。行列式就是这个平行四边形的面积（带正负号）。
...
>
> 实际中不会这么极端，但因子之间高度相关（接近共线性）时，行列式接近 0，回归结果会变得不稳定。Day 18 会详细讲这个问题。
...
> 秩 $<$ 列数 → 多重共线性问题。Day 18 会详细讲怎么检测和处理。
...
$$r_t = \alpha + b_1 \cdot MKT_t + b_2 \cdot SMB_t + b_3 \cdot HML_t + e_t$$
...
- $b_1, b_2, b_3$：因子载荷（这只股票对每个因子的敏感度）
- $e_t$：残差（噪声）
...
> **翻译成人话：** 这个公式从 60 个月的因子数据和收益率数据中，算出最优的 4 个系数 $[\alpha, b_1, b_2, b_3]$。
>
> $\alpha$ 是否显著不为零 → 这只股票有没有"因子解释不了的超额收益"。
> $b_1, b_2, b_3$ 的大小 → 这只股票对市场、规模、价值因子的敏感度。
...
- [ ] 我知道矩阵就是把多个向量排成一个表格
- [ ] 我能做维度检查：$(m \times n)(n \times p) = (m \times p)$
- [ ] 我能手算 $2 \times 2$ 矩阵乘以 $2 \times 1$ 向量
- [ ] 我知道行列式 $= 0$ 意味着不可逆（信息丢失、因子共线）
- [ ] 我能手算 $2 \times 2$ 矩阵的行列式和逆矩阵
...
| 内容 | 为什么不需要 |
|------|------------|
| $3 \times 3$ 以上矩阵的行列式计算 | 实际中用 `numpy.linalg.det()` |
| $3 \times 3$ 以上矩阵的逆矩阵计算 | 实际中用 `numpy.linalg.inv()` 或更好的 `numpy.linalg.solve()` |
