...
> **需要记住的：** $\rho = \text{Cov}(X,Y) / (\sigma_X \cdot \sigma_Y)$，范围 $[-1, +1]$。Day 07 介绍了相关系数的直觉，今天补上了数学定义。
...
把 §1.6 的 $\text{Var}(X+Y)$ 公式加上权重，并用 $\text{Cov} = \rho \sigma_X \sigma_Y$ 替换协方差，就得到 Day 07 §3.2 的组合方差公式：

$$\sigma_p^2 = w_1^2 \sigma_1^2 + w_2^2 \sigma_2^2 + 2 w_1 w_2 \rho \sigma_1 \sigma_2$$

Day 07 直接给了这个公式，今天从协方差推导出来了——**Day 07 的"给定结论"变成了 Day 15 的"推导结果"。**

**手算验证：复用 Day 07 的数据**

Day 07 §3.3 的设定：股票 A（$\sigma_A = 25\%$），股票 B（$\sigma_B = 15\%$），等权（$w_A = w_B = 0.5$）。
...
> 这和 Day 07 §3.3 的结果完全一致——今天用协方差和相关系数从数学上推导出了同样的结论。
...
- [ ] 我理解了 Day 06 分散投资的数学本质（协方差为负 → 组合方差减小）
