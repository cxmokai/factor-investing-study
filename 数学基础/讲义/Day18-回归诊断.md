...
Day 19 会介绍时间序列回归和横截面回归这两种视角的区别——届时你会看到，每个横截面内部不需要 NW。
...
| 得分 | 状态 |
|------|------|
| 9-10 题 | 回归诊断掌握扎实，明天可以直接学时间序列基础与横截面视角 |
| 7-8 题 | 不错，回看错误的部分 |
...
Day 17-18 的回归有一个隐藏前提：**输入数据必须是平稳的。** 如果你拿股票价格（非平稳）直接做回归，会得到看起来很好但完全虚假的结果——这叫伪回归。

明天你将学到：

- **平稳性与差分**：为什么因子模型必须用收益率而不是价格
- **自相关函数 ACF**：动量和均值回复的数学本质
- **横截面回归**：除了时间序列回归（Day 17-18），还有另一种切数据的方式——同一个月 × 多只股票

> Day 17 学了"怎么做回归"，Day 18 学了"怎么做一个靠谱的回归"，Day 19 学"回归的输入数据要满足什么条件，以及数据可以怎么切"。