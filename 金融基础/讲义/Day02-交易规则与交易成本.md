...
> 连续竞价阶段盘中价再偏离开盘价 ±20%，即 $(1+20\%) \times (1+20\%) - 1 = 44\%$。
...
|-----------|---------------|---------|---------|
| 交易成本（印花税+佣金+滑点） | **回测必须扣成本** | 很多策略扣完成本后不赚钱；调仓频率越高成本越大 | 编程实战阶段 |
| T+1 规则 | **调仓频率限制** | A 股无法日内交易，因子策略通常按周/月调仓，影响不大 | 编程实战阶段 |
| 涨跌停板（10%/20%） | **极端行情下的流动性风险** | 跌停时卖不出去，因子策略需要考虑停牌/跌停股的处理 | 编程实战阶段 |
| 收盘价（集合竞价） | **因子信号和执行价格** | 因子策略通常在收盘价附近调仓，集合竞价影响执行质量 | 编程实战阶段 |
| CEX vs DEX | **加密因子数据来源** | 因子投资主要用 CEX（流动性好、API 完善、费用低） | 编程实战阶段 |
| 加密 24/7 交易 | **日频数据的定义** | 需要选定 UTC 0:00 做"日快照"，与 A 股收盘价对齐 | 编程实战阶段 |
| 退市/归零风险 | **生存偏差（Survivorship Bias）** | 回测数据库必须包含已退市的股票/归零的币，否则高估策略收益 | 编程实战阶段 |
...
**Day 03** 开始学财务报表——三张表分别回答什么问题、怎么看懂一家公司的"体检报告"。这是构造价值因子和质量因子的基础。
