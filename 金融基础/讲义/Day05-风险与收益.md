...
**数字货币的例子：** 你以 40 万元买了 1 枚 BTC，一个月后涨到 44 万元：

```
R = (44 - 40) / 40 = 4 / 40 = 10%
```

> 注意：数字货币**没有分红**，所以上面带分红 $D$ 的公式不适用于 BTC。数字货币的收益完全来自价格变动。
...
用 BTC 感受一下"差异很大"是什么意思——BTC 单月涨 50% 并不罕见：

```
BTC 某月从 30 万涨到 45 万：
  简单收益率：R = (45 - 30) / 30 = 50%
  对数收益率：r = ln(45 / 30) = ln(1.5) ≈ 40.5%

差了近 10 个百分点！收益率越大，两种算法差异越大。
```
...
> **数字货币的年化：用 $\sqrt{365}$ 而非 $\sqrt{250}$**
>
> 股票一年交易约 250 天，但数字货币 **24/7 全年无休**，一年交易 365 天。所以 BTC 的年化波动率用 $\sqrt{365} \approx 19.1$：
>
> ```
> BTC 日波动率 ~3.5% → 年化 = 3.5% × √365 ≈ 3.5% × 19.1 ≈ 66.9%
> ```
>
> 同样的日波动率，用 $\sqrt{365}$ 比 $\sqrt{250}$ 算出来更大——因为全年无休意味着波动有更多天数"积累"。
...
| 单只 A 股 | 30 - 60% | 剧烈波动 |
| 比特币（BTC） | 50 - 80% | 极端波动，但在加密货币中最"稳" |
| 以太坊（ETH） | 70 - 100% | 比 BTC 更高 |
| 山寨币（Altcoins） | 100 - 200% | 超极端波动，归零风险不可忽略 |
...
标准差只看"偏离平均值多远"，**不区分涨和跌**。举个例子：

```
股票 A：连续 5 天收益率  +5%, +3%, +6%, +4%, +7%  → 标准差 ≈ 1.4%
股票 B：连续 5 天收益率  -5%, -3%, -6%, -4%, -7%  → 标准差 ≈ 1.4%
```

两只股票的波动率算出来一模一样，但 A 天天涨、B 天天跌——你的体验完全不同。

**问题就在这里：** 标准差把"好的波动"（上涨）和"坏的波动"（下跌）混在一起算了。作为投资者，你讨厌的是下跌，而不是上涨。一只涨得猛的股票被标记为"高风险"，这不太合理。

这是波动率最大的批评之一。后续会学到"下行风险"等指标来弥补——它们只计算下跌方向的波动，忽略上涨，更符合投资者的真实感受。今天只需知道这个局限性存在。
...
上一节我们学了波动率——它告诉你收益率"平均偏离多远"。但投资者更关心的问题往往是：**最坏的情况下，我到底能亏多少钱？** 这就是 VaR 要回答的问题。
...
**核心思路：** VaR 要回答的是"往坏的方向走多远才算极端"。如果收益率服从正态分布，那我们只需要知道：从平均值往左（亏损方向）走几个标准差，就能划出"最坏 5%"的分界线。

**第一步：理解"左尾 5%"和分位数 z**

正态分布是一条钟形曲线，中间高、两边低。我们把曲线下方的面积看作 100% 的可能性：

<svg width="480" height="240" viewBox="0 0 480 240" xmlns="http://www.w3.org/2000/svg">
  <!-- 背景 -->
  <rect width="480" height="240" fill="#fafafa" rx="8"/>
  <!-- 标题 -->
  <text x="240" y="24" text-anchor="middle" font-size="14" font-weight="bold" fill="#333">正态分布（钟形曲线）</text>
  <!-- 坐标轴 -->
  <line x1="40" y1="190" x2="440" y2="190" stroke="#666" stroke-width="1.5"/>
  <!-- 钟形曲线路径 (正态分布) -->
  <path d="M40,190 C60,189 80,188 100,185 C120,180 140,170 160,155
           C180,135 195,105 210,75 C220,55 228,42 240,38
           C252,42 260,55 270,75 C285,105 300,135 320,155
           C340,170 360,180 380,185 C400,188 420,189 440,190"
        fill="none" stroke="#4a90d9" stroke-width="2.5"/>
  <!-- 左尾 5% 填充区域 -->
  <path d="M40,190 C40,190 80,188 100,185 C112,182 122,177 132,172
           L132,190 Z"
        fill="#e74c3c" fill-opacity="0.35" stroke="#e74c3c" stroke-width="1"/>
  <!-- 分界线 z = -1.65σ -->
  <line x1="132" y1="172" x2="132" y2="198" stroke="#e74c3c" stroke-width="2" stroke-dasharray="4,3"/>
  <!-- 均值线 -->
  <line x1="240" y1="38" x2="240" y2="198" stroke="#999" stroke-width="1" stroke-dasharray="4,3"/>
  <!-- 标注：左尾 5% -->
  <text x="80" y="182" text-anchor="middle" font-size="12" font-weight="bold" fill="#e74c3c">5%</text>
  <!-- 标注：95% 区域 -->
  <text x="286" y="140" text-anchor="middle" font-size="13" fill="#4a90d9">95%</text>
  <!-- 标注：均值 -->
  <text x="240" y="215" text-anchor="middle" font-size="12" fill="#666">均值 (0)</text>
  <!-- 标注：z 分界线 -->
  <text x="132" y="220" text-anchor="middle" font-size="11" font-weight="bold" fill="#e74c3c">−1.65σ</text>
  <!-- 箭头指向分界线 + 说明 -->
  <text x="68" y="214" text-anchor="start" font-size="10" fill="#e74c3c">↑ 分界线</text>
</svg>

- 左尾那一小块阴影面积 = 5%，代表"最差的 5% 情况"
- **分界线的位置在均值左边 1.65 个标准差处**——这个 1.65 就是 $z$
- 这不需要你去算，它是从正态分布表查出来的固定数字：
  - 想划出最差 5%（95% 置信）→ $z = 1.65$
  - 想划出最差 1%（99% 置信）→ $z = 2.33$

**第二步：从 z 推导出公式**

既然"最坏情况 = 往亏损方向走 $z$ 个标准差"，那：

$$\text{最坏日收益率} = z \times \text{日波动率}$$

乘上投资金额，就知道最多亏多少钱：

$$\text{VaR} = \text{投资金额} \times z \times \text{波动率}$$
...
95% 日 VaR = 100万 × 1.65 × 1.5%
           = 100万 × 0.02475
           = 2.475 万元

含义：在正常市场下，95% 的交易日你的亏损不会超过 2.5 万。
...
```
BTC 日波动率约 3.5%（远高于沪深 300 的 1.5%）

95% 日 VaR = 100万 × 1.65 × 3.5%
           = 100万 × 0.05775
           = 5.775 万元

同样的 100 万，BTC 的日 VaR 是沪深 300 的 2.3 倍（5.8 万 vs 2.5 万）。
也就是说，投 BTC 每 20 天就有一天可能亏掉近 6 万——这就是高波动资产的代价。
```
...
| 问题 | 说明 |
|------|------|
| **不告诉你尾部风险** | VaR 只说"95% 的情况下最多亏 X"，但剩下 5% 可能亏多少？不知道。用上面的例子：VaR 说最多亏 2.5 万，但那 5% 的日子里，实际可能亏 5 万（2 倍），甚至 25 万（10 倍） |
...
- [x] 能写出简单收益率和对数收益率的公式
- [x] 知道对数收益率的两个优势（可加性、统计分布）
- [x] 知道量化投资为什么偏好对数收益率
- [x] 能用年化公式把 n 年总收益换算成年化收益率
- [x] 理解标准差衡量的是"偏离平均值的程度"
- [x] 知道年化波动率 = 日波动率 × $\sqrt{250}$
- [x] 知道 VaR 的含义——在给定置信水平下的最大可能损失
- [x] 知道 VaR 的主要局限（不反映尾部风险、假设正态分布）
...
BTC 长期夏普比率     ≈ 0.5 - 0.8（收益极高但波动也极大，夏普并不突出）
```

> **BTC 的夏普比率为什么没有想象中高？** 虽然 BTC 年化收益率远超股票（50-80% vs 10%），但它的年化波动率也远超股票（60-80% vs 20%）。高收益被高波动"除掉"了，夏普比率反而和股票市场差不多。这正好说明了夏普比率的价值——它穿透了"绝对收益"的表象，告诉你每单位风险获得了多少回报。
...
BTC 历史上多次经历 -80% 以上的回撤——每次都让人以为"这次是真的归零了"，但每次都恢复并创新高：

```
2011 年：$32 → $2       回撤 -94%    恢复用时 ~2 年
2013-2015：$1,100 → $170   回撤 -85%    恢复用时 ~3 年
2017-2018：$19,800 → $3,200  回撤 -84%    恢复用时 ~3 年
2021-2022：$69,000 → $15,500 回撤 -77%    恢复用时 ~2 年
```

> 对照上面"亏损不对称"的表格：-80% 的回撤需要涨 **400%** 才能回本。BTC 每次都做到了——但中间等待 2-3 年，绝大多数人在恐惧中早已割肉离场。这就是为什么"长期持有 BTC 回报惊人"在事后看很容易，事前做到几乎不可能。
...
比特币（BTC）自 2009 年诞生以来，已经从极客玩具成长为一类不可忽视的资产。用我们今天学的工具来审视它：

| 指标 | 大约值 | 备注 |
|------|:-----:|------|
| BTC 年化收益率（2013-2024） | ~50-80% | 取决于起止时间，差异极大 |
| 年化波动率 | 60-80% | 远高于任何传统资产 |
| 夏普比率 | ~0.5-0.8 | 与股票类似，但不同时段差异巨大 |
| 最大回撤 | 多次 -80% 以上 | 见上方 §6.1 的历史回撤案例 |
...
```
BTC 早期投资者的回报是天文数字：
  2013 年初 1 BTC ≈ $13 → 2024 年底 ≈ $90,000+
  但中间多次跌去 80% 以上，大多数人根本拿不住
```
...
```
              年化收益    年化波动率    夏普比率
标普 500       ~10%       ~20%        ~0.4
BTC           ~50-80%    ~60-80%     ~0.5-0.8
```

BTC 的收益远高于股票，但波动也远高于股票。有趣的是，夏普比率并没有高出太多——高收益的"代价"是极端的波动。

**3. 数字货币历史太短**

BTC 只有约 15 年历史，远不如百年美股数据可靠。我们无法确定过去的高收益是否会持续——也许只是新资产类别早期的"发现溢价"。

> **与因子投资的联系：** 数字货币市场正在成为因子投资的新战场。动量因子在加密货币中表现尤其显著——过去涨得多的币往往继续涨（当然崩起来也更猛）。但由于历史数据短、市场结构不成熟，因子策略在加密货币中的可靠性仍有争议。
...
graph TD
    ...
    F["比特币<br>收益：50-80%<br>波动：60-80%<br>夏普：~0.5-0.8"]
    end
    A --> B --> C --> D --> E --> F
```
...
- [x] 知道无风险利率的含义和中美的大致水平
- [x] 能说出风险溢价的公式和直觉含义
- [x] 知道夏普比率的公式：$\text{Sharpe} = (R_p - R_f) / \sigma_p$
- [x] 理解夏普比率衡量的是"每承担一单位风险获得的超额收益"
- [x] 知道夏普 1.0 已经算优秀
- [x] 知道最大回撤的定义和"亏损不对称"的道理
- [x] 知道美国大盘股长期年化约 10%，波动约 20%
- [x] 理解"风险溢价不是白拿的"这个核心教训
...
自测 11：BTC 日波动率 3.5%，且 24/7 全年无休交易，年化波动率是多少？
→ 你的答案：_______________
→ 正确：年化波动率 = 3.5% × √365 ≈ 3.5% × 19.1 ≈ 66.9%。
  数字货币全年 365 天交易，所以用 √365 而非股票的 √250。
```

**10-11 道对：** 完美，收益与风险度量的基本工具已经掌握。
**8-9 道对：** 不错，回看错的部分就好。
**7 道以下：** 建议花 20 分钟重新阅读讲义，重点看公式和例子。
