...
Day 06 解决的问题是：**手里有一个风险资产和一个无风险资产，怎么分配比例。** 工具是资本配置线（CAL）。
...
Day 06 的雨伞-冰淇淋例子说明了分散化的直觉。现在给这个直觉加上数学。
...
> **"为什么说分散化是投资中唯一的免费午餐？"** 这句话来自诺贝尔奖得主 Markowitz。通常在投资中，降低风险必须牺牲收益（Day 05 讲过的风险-收益权衡）。但分散化是例外——它在不降低预期收益的情况下降低风险。前提是资产之间不是完全正相关的。
...
>
> **关键概念：投资者承担系统性风险会获得补偿（风险溢价），但承担非系统性风险不会获得补偿——因为后者可以免费消除。** 这是 Day 08 CAPM 的核心逻辑预告。
...
> **所有理性投资者都应该选择有效前沿上的组合。** 具体选前沿上的哪个点，取决于个人的风险厌恶程度（Day 06 的效用函数）。
...
Day 06 学了资本配置线（CAL）——从无风险利率出发，连到一个风险组合，画一条直线。
...
> Day 06 的结论是"选夏普比率最高的风险资产画 CAL"。今天的结论是这个"最高夏普比率的风险资产"**就是有效前沿上的切线组合**。Day 06 和 Day 07 的知识在这里合流了。
...
> **"市场组合"在理论上包含世界上所有的可交易资产——所有股票、所有债券、所有房产等。** 实际中用市场指数（如沪深 300、标普 500）近似。这是一个理论概念，Day 08 的 CAPM 模型会详细讲。
...
**第 2 步（因人而异）：** 根据自身风险厌恶程度，在最优风险组合和无风险资产之间分配比例（Day 06 的 $y^*$ 公式）。
...
```mermaid
graph TD
    A["Day 05：衡量单个资产<br>收益率、波动率、夏普"] --> B["Day 07：找最优风险组合<br>相关系数 → 有效前沿 → 切线组合"]
    B --> C["Day 06：决定配置比例<br>效用函数 → CAL → y*"]
    C --> D["完整的投资组合"]
...
当一个新资产和组合中现有资产的相关性低时，即使它本身波动很大，少量配置也能降低组合整体的波动。这就是 Day 06 中 BTC 最优配比虽然不高（~5-13%）但确实为正的原因。
...
> **但这里有一个陷阱：相关性不稳定。**
> 上面的分析假设相关系数是稳定的，但现实中 BTC 和股票的相关性在危机时期会**突然飙升**（从 +0.3 跳到 +0.7 以上）。这意味着你指望 BTC 帮你分散风险的时候——恰恰是市场恐慌、所有资产一起跌的时候——分散化效果可能大打折扣。这是"**相关性不稳定**"问题，在 Day 06 的 BTC 配置讨论中也提到过。
...
| ----------------------- | ------------------------ | --------------------------------------------------------------------------------------- | ------------ |
| 相关系数                | **多因子组合的核心**     | 价值因子和动量因子相关性低（甚至为负），组合后风险大幅降低，夏普显著提升                | Day 09-10     |
| 组合波动率公式          | **因子组合风险计算**     | 回测多因子策略时用此公式（矩阵版）计算组合的预期波动率                                  | 编程实战阶段 |
| 系统性 vs 非系统性风险  | **因子策略的风险分解**   | 因子策略自动分散了非系统性风险，剩余的是系统性风险和因子风险                            | Day 08        |
...
| MPT 的局限              | **因子模型的必要性**     | 直接估计股票间相关系数太难，因子模型通过少量因子间接推算——这正是 Day 09 多因子模型的动机 | Day 09        |

> 模块四（投资组合理论）至此完结。Day 06-07 告诉你"怎么把资产组合在一起"，接下来的模块五将回答一个更根本的问题："资产的收益率到底是由什么决定的？"——这就是 CAPM 和多因子模型的领域，也是因子投资理论的核心。

---

## 明天预告

**Day 08** 进入全新且最核心的模块——**资产定价模型（CAPM）**。
...
> CAPM 是理解因子投资的基石。Day 08-10 是整个金融基础部分的核心中的核心。
