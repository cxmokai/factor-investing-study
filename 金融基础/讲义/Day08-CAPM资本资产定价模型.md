...
Day 07 解决的问题是：**怎么把多个资产组合在一起，找到最优的风险组合。** 答案是有效前沿上的切线组合（夏普比率最高的那个）。分离定理告诉我们，如果所有人都理性，这个最优风险组合就是市场组合。

但这留下了一个更根本的问题：**如果市场组合是最优的，那么单个资产的合理价格应该是多少？一只股票的预期收益率应该是多少？**

Day 07 是"从下往上"——把资产组合起来。Day 08 是"从上往下"——从市场组合出发，反推每个资产应该有多少回报。
...
Day 07 讲了两种风险：系统性风险（消不掉的市场风险）和非系统性风险（可以分散掉的个股风险）。CAPM 的核心观点是：
...
| 国债 | 接近 0 | 几乎不受股票市场影响 |
| **BTC**（vs 标普 500） | 约 1.0-1.5 | **极不稳定**——2020 年前接近 0，之后随机构入场而上升 |

> **BTC 的 Beta 为什么不稳定？** BTC 早期和传统市场几乎无关（$\beta \approx 0$），但随着机构投资者入场、BTC ETF 获批，它和股票市场的联动性大幅增加。这种 Beta 的剧烈变化本身就是 CAPM "Beta 稳定"假设被违反的绝佳例证——你用过去 2 年算出的 Beta 来预测未来的收益，可能完全不靠谱。
...
![Beta 回归散点图](assets/day08-beta-regression.svg)
...
Day 07 学过协方差——衡量两个资产"一起动"的程度。Beta 其实就是**把协方差标准化**——除以市场自身的波动程度。
...
>
> **为什么会有风险溢价？** 因为人是厌恶风险的（Day 06 讲的风险厌恶）。要让人愿意承担市场波动的风险，就必须提供比无风险利率更高的预期收益作为补偿。如果股市的预期收益和银行存款一样，没人愿意承担风险去炒股。
...
```mermaid
graph TD
    A["Day 06-07：所有投资者都<br>用均值-方差分析"] --> B["分离定理：所有人持有<br>相同的最优风险组合"]
...
![证券市场线（SML）](assets/day08-sml.svg)
...
Day 07 学了资本市场线（CML），今天又学了证券市场线（SML）。两者容易混淆，用表格区分：
...
| 来源 | Day 07：无风险资产 + 市场组合 | Day 08：CAPM 模型 |

> **关键区别：** CML 的横轴是总风险（$\sigma$），只有有效组合才在线上。SML 的横轴是 Beta（系统性风险），**所有资产**在均衡时都应该落在线上。SML 比 CML 更通用——它告诉你任何一个资产的期望收益应该是多少。

> **为什么 SML 用 Beta 而不用总风险 $\sigma$？** 因为非系统性风险可以通过分散化免费消除（Day 07 的结论），所以市场不会为它提供补偿。只有消不掉的系统性风险（用 Beta 衡量）才会获得补偿。两个总风险相同但 Beta 不同的资产，期望收益不同——Beta 高的那个期望收益更高。
...
![Alpha：SML 上方与下方](assets/day08-alpha-sml.svg)
...
> **换句话说：** CAPM 是"一因子模型"——它只用市场因子（Beta）来解释收益。但一个因子不够用。Day 09 会学的 Fama-French 三因子模型加入了市值因子和价值因子，能解释更多的收益来源。Day 10 会学五因子模型。因子模型越完善，能解释的收益越多，剩下的"真正的 Alpha"就越少。
...
如果把 CAPM 套用到加密市场，"市场组合"用什么代替？通常用 **BTC** 作为加密市场的市场代理（就像用沪深 300 代理 A 股市场）。

**示例：用 CAPM 估算 ETH 的预期收益**

$$E(r_{ETH}) = R_f + \beta_{ETH/BTC} \times [E(r_{BTC}) - R_f]$$

参数代入：$R_f = 4\%$，$\beta_{ETH/BTC} \approx 1.2$，$E(r_{BTC}) - R_f \approx 26\%$

$$E(r_{ETH}) = 4\% + 1.2 \times 26\% = 35.2\%$$

如果 ETH 的实际收益远高于 35.2%，说明 ETH 有正的 Alpha——超越了 Beta 预测的收益。

**但 CAPM 在加密市场的问题比传统市场更严重：**

| 问题 | 传统市场 | 加密市场 |
|------|---------|---------|
| 市场组合 | 用沪深 300/标普 500 近似，虽不完美但可接受 | BTC 市值占比波动大（30-70%），代理效果不稳定 |
| 收益率分布 | 近似正态，有肥尾 | 严重偏离正态——暴涨暴跌的频率远高于正态分布预测 |
| Beta 稳定性 | 虽会变化，但跨年度相对稳定 | 跨牛熊剧烈变化——同一个代币在牛市和熊市的 Beta 可能差 2-3 倍 |
| 投资者同质性 | 假设所有人预期相同，虽不真实但差距有限 | 散户、鲸鱼、矿工的预期和行为差异极大 |

> **CAPM 的核心概念在加密市场仍然适用：**
> - **Beta**：衡量代币对 BTC 的敏感度——DeFi 蓝筹（如 UNI）的 $\beta$ 约 1.0-1.3，Meme 币的 $\beta$ 可能高达 2-3
> - **Alpha**：超越 Beta 预测的收益。如果某代币长期跑赢其 Beta 对应的预期收益，说明有正 Alpha——这正是加密因子投资要捕获的东西
> - **系统性风险 vs 非系统性风险**：持有 5-10 个不同赛道的主流代币可以分散掉大部分非系统性风险，但 BTC 下跌带来的系统性风险无法分散
...
> **不理解 CAPM，就无法理解后面的多因子模型。** CAPM 告诉你什么是"正常的收益"（Beta 带来的），多因子模型告诉你"正常之外还有什么"。Day 09 就会进入这个更精彩的领域。
...
| 多因子模型的具体公式 | 知道"在 CAPM 上加因子"的概念就行 | Day 09-10 |
...
|-----------|---------------|---------|---------|
| Beta | **市场因子（MKT）** | Beta 是市场因子的载荷（factor loading），因子模型中的第一个也是最基本的因子 | Day 09 |
| 市场风险溢价 | **因子风险溢价** | 每个因子都有自己的"风险溢价"——承担因子风险获得的补偿。市场风险溢价是第一个例子 | Day 09-10 |
| CAPM 公式 | **因子模型的原型** | 三因子模型就是在 CAPM 右边加两个因子项：$E(r) = r_f + \beta_{\text{MKT}} \times \text{MKT} + \beta_{\text{SMB}} \times \text{SMB} + \beta_{\text{HML}} \times \text{HML}$ | Day 09 |
| Alpha | **因子投资的目标** | 因子投资者追求的就是 Alpha——CAPM 解释不了的超额收益 | Day 09-10 + 编程实战阶段 |
| SML 上方/下方 | **因子选股的理论基础** | 选出"SML 上方"的股票（正 Alpha）就是在选因子暴露高的股票 | 编程实战阶段 |
| CAPM 的异常现象 | **经典因子的来源** | 小盘股效应 → 市值因子，价值股效应 → 价值因子，动量效应 → 动量因子 | Day 09-10 |
| CAPM 的单因子局限 | **多因子模型的动机** | 一个因子解释不够 → 加更多因子 → 三因子、五因子模型 | Day 09 |
...
**Day 09** 进入多因子模型——**APT（套利定价理论）与 Fama-French 三因子模型**。
