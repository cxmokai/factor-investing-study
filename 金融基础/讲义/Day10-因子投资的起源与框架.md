...
> **节奏：** 每 50 分钟休息 10 分钟
>
> **今日目标：** 掌握从三因子到五因子的发展脉络，理解动量因子的特殊地位，了解"因子动物园"问题，建立因子投资从学术到实践的完整框架——今天是金融基础部分的收官整合课
>
...
00:50 - 01:00  休息
01:00 - 01:50  [第2节] 因子投资的完整框架 + Day 01-10 知识串联
01:50 - 02:00  [收尾] 自测
```
...
但 Day 09 末尾留了一个问题：**三因子模型也有解释不了的现象——最突出的是动量效应。**
...
> **这是为什么动量因子虽然平均收益最高，却不能只靠它。** 动量策略需要和其他因子（尤其是价值因子）组合——因为动量崩溃往往发生在价值因子大幅跑赢的时候，两者天然对冲。这就是 Day 07 学的"分散化"在因子层面的应用。
...
>
> **和 Day 04 的联系：** 盈利因子用到的"营业利润率"就是 Day 04 学的利润表中的概念。你在 Day 04 学的财务指标，在这里变成了一个因子的构造基础。
...
>
> **和 Day 03 的联系：** 投资因子用到的"总资产增长率"来自 Day 03 学的资产负债表。资产负债表上总资产的变化，直接决定了这个因子的构造。
...
学了 10 天金融基础，现在终于可以给"因子投资"一个完整的定义了。
...
| 构造投资组合 | 不是买单只股票，而是构建一篮子 | Day 06-07 的投资组合理论 |
| 暴露在因子上 | 让组合对目标因子有高载荷 | Day 09 的因子载荷概念 |
| 已被验证 | 经过严格的学术和实证检验 | 上一节的因子筛选标准 |
| 长期超额收益 | 长期来看能跑赢市场 | Day 08 的 Alpha 概念 |
| 因子溢价 | 承担因子风险获得的补偿 | Day 09 的因子溢价概念 |
...
| 考虑因素 | 说明 |
|---------|------|
| 因子之间的相关性 | 选相关性低的因子组合，分散化效果好（Day 07 的原理） |
| 交易成本 | 动量因子换手率高 → 交易成本高；价值因子换手率低 → 成本低 |
| 市场适用性 | 在 A 股，市值因子和价值因子效果好；动量因子需要注意 T+1 限制 |
| 数据可获取性 | 有些因子需要的数据容易获取（市值、PE），有些较难（盈利预期） |

> **经典组合：价值 + 动量。** 这两个因子的相关性为负（价值因子好的时候动量因子往往差，反之亦然）——Day 07 学过，相关性低甚至为负的资产组合，风险降低最多。这是多因子投资中最经典的搭配。
...
| 步骤 | 关键问题 | 对应知识 |
|------|---------|---------|
| 回测 | 策略在历史上表现如何？夏普比率多高？最大回撤多大？ | Day 05 的收益/风险指标 |
| 实盘 | 多久调仓一次？交易成本怎么控制？ | Day 02 的交易成本 |
| 归因 | 这个月赚的钱来自哪个因子？Alpha 是多少？ | Day 08-09 的因子模型回归 |
...
| 数据获取 | CoinGecko、CoinMarketCap、CCXT、Glassnode、Dune Analytics |
| 产品 | 极少——几乎是空白市场（对比股票 Smart Beta 市场 $1.5T+） |
| 限制 | 数据历史短、生存偏差严重、缺乏基本面数据 |

**数据来源详解：**

| 数据类型 | 来源 | 说明 |
|---------|------|------|
| 价格和市值 | CoinGecko API、CoinMarketCap | 免费，覆盖数千种代币的历史数据 |
| 交易所数据 | CCXT（统一 API） | 一个 Python 库统一接入 100+ 交易所，获取 K 线、订单簿、交易记录 |
| 链上数据 | Glassnode、Dune Analytics | 活跃地址、交易量、持仓分布等——传统市场没有的"透视"数据 |
| 开发者数据 | Santiment、GitHub API | 代码提交频率、开发者人数——衡量项目是否还"活着" |

**实操中的关键差异：**

和传统股票因子投资相比，加密市场有几个特殊问题：

1. **投资宇宙定义：** 不能无脑选"所有代币"。通常选 top 100-200 按市值排名的代币，并排除稳定币（USDT、USDC）和 wrapped token（WBTC）。低流动性的代币无法实际交易
2. **调仓频率 vs gas 费：** 理论上可以日频调仓（24/7 交易），但链上交易的 gas 费会吃掉收益。CEX 交易手续费相对低（0.1%），但涉及资产托管风险
3. **生存偏差是最大挑战：** 2017 年 ICO 热潮中的代币，超过 90% 已经归零。如果你的回测数据只包含"现在还活着"的币，策略收益会被严重高估

**一个简单策略示例——月度动量：**

```
步骤 1：每月初，选取市值 top 50 的非稳定币代币
步骤 2：计算每个代币过去 30 天的收益率
步骤 3：买入收益率最高的 10 个，等权持仓
步骤 4：持有 30 天后清仓，重复以上步骤
```

这个策略在 2020-2021 年牛市中的回测表现惊人（年化可达数百%），但在 2022 年熊市中亏损同样巨大（-70% 以上）。**没有经过完整牛熊周期验证的策略，不值得信赖。**

> **加密市场的因子投资是一片蓝海。** 传统市场的 Smart Beta 产品规模已超过 $1.5 万亿，有成百上千的 ETF 产品。而加密市场几乎为零——这既是机会（因子溢价可能更大、竞争更少），也是风险（数据质量差、市场规律不稳定、监管不确定性大）。编程实战阶段我们会动手构建加密因子策略。
...
```mermaid
graph TD
    A["Day 01-02：市场基础<br>知道你在交易什么<br>交易成本怎么算"] --> B["Day 03-04：财务报表<br>看懂 PE/PB/ROE<br>构造价值/质量因子的数据基础"]
    B --> C["Day 05：风险与收益<br>收益率、波动率、夏普比率<br>评估因子和策略的工具"]
    C --> D["Day 06-07：投资组合理论<br>分散化、有效前沿、分离定理<br>多因子组合的理论基础"]
    D --> E["Day 08：CAPM<br>Beta、Alpha、SML<br>因子投资的坐标原点"]
    E --> F["Day 09：三因子模型<br>SMB、HML<br>因子投资的理论基石"]
...
金融基础部分（模块五）在今天基本收官。还剩一天——Day 11 的行为金融——解释**为什么因子溢价能长期存在**。

之后你将进入：

| 阶段 | 内容 | 和因子投资的关系 |
|------|------|----------------|
| **数学基础**（10 天） | 线性代数、概率统计、时间序列 | 因子模型回归、组合优化、统计检验的数学基础 |
| **因子投资**（18 天） | 因子构造、回测方法论、多因子策略 | 深入学习每个因子的构造细节和组合方法 |
| **编程实战**（30 天） | Python 数据分析、因子构造、回测 | 把所有理论用代码实现 |

> **金融基础是"知道做什么"，数学是"知道怎么算"，因子投资是"知道怎么用"，编程是"真正去做"。** 四者缺一不可。你现在已经完成了第一个阶段。
...
- [ ] 能把 Day 01-10 的知识串联成一条完整的逻辑链
- [ ] 知道接下来的学习路径（数学基础 → 因子投资 → 编程实战）
...
自测 10：Day 03-04 学的财务报表知识在因子投资中怎么用？
→ 你的答案：_______________
...
| 组合优化的数学细节 | 知道"找最优权重"的概念就行 | 数学基础阶段 + 编程实战阶段 |
...
> 模块五（资产定价模型）至此完结。Day 08-10 三天建立了因子投资的完整理论框架：CAPM 定义了 Alpha 和 Beta → 三因子模型发现了市值和价值两个独立收益来源 → 五因子模型加上盈利和投资 → 动量因子补齐拼图。现在你已经知道"因子投资是什么、为什么有效、怎么做"——剩下的就是 Day 11 回答"为什么因子溢价能长期存在"，然后进入数学和编程阶段把理论变成现实。

---

## 明天预告

**Day 11** 是金融基础部分的最后一天——**行为金融与因子的关系**。
