...
| 表 | 核心问题 | 类比 | 时间特征 |
|---|---------|------|---------|
| **资产负债表** | 公司**有什么**（资产）、**欠什么**（负债）、股东**真正拥有多少**（净资产） | 你的身体成分报告：骨骼、肌肉、脂肪各多少 | **时间点**（比如 12月31日那天的状态） |
| **利润表** | 这段时间**赚了多少钱**，怎么赚的 | 你的健身成绩单：这个月减了多少脂肪、增了多少肌肉 | **时间段**（比如 1月1日到12月31日） |
| **现金流量表** | 实际**收到/花出**了多少现金 | 你银行卡的流水：实际进了多少钱、出了多少钱 | **时间段** |
...
|------|------|------|---------|---------|---------|
| **PB**（市净率） | Price-to-Book ratio | 股价 / 每股净资产 | 股票市场 | **资产负债表** | 家底贵不贵 |
| **PE**（市盈率） | Price-to-Earnings ratio | 股价 / 每股收益(EPS) | 股票市场 | **利润表** | 赚钱能力贵不贵 |
| **ROE**（净资产收益率） | Return On Equity | 净利润 / 净资产 | **利润表** | **资产负债表** | 股东的钱用得好不好 |
...
> **小白提示：** 你现在不需要真去看。后面编程实践阶段会用代码自动获取财务数据（通过 Tushare 或 AkShare 等 Python 库），比手动翻方便得多。今天只需要理解财报的结构和含义。
...
**EPS（每股收益，Earnings Per Share）= 净利润 / 总股数**
...
|------|---------|---------|---------|---------|
| **价值因子** | PE、PB | 利润表 + 资产负债表 | 买低估卖高估 | 第 8 周 |
| **质量因子** | ROE、毛利率 | 利润表 + 资产负债表 | 买高质量卖低质量 | 第 10 周 |
| **市值因子** | 市值 | 资产负债表（总股数）+ 股价 | 买小盘卖大盘 | 第 7 周 |
...
|-----------|---------------|---------|---------|
| 净资产（所有者权益） | **PB → 价值因子（Value）** | PB = 股价 / 每股净资产；低 PB 组长期跑赢高 PB 组 | 第 8 周 |
| 净利润 | **PE → 价值因子（Value）** | PE = 股价 / EPS；低 PE 组长期跑赢高 PE 组 | 第 8 周 |
| EPS（每股收益） | **PE 的分母** | 因子投资通常用 TTM PE（最近 12 个月滚动） | 第 8 周 |
| ROE（净资产收益率） | **质量因子（Quality）核心** | 高 ROE = 赚钱效率高；Fama-French 五因子模型专门捕捉盈利溢价 | 第 10 周 |
| 毛利率 | **质量因子（Quality）** | 毛利率高且稳定 = 竞争护城河强，质量因子筛选常用 | 第 10 周 |
| 扣非净利润 | **因子计算用的利润口径** | 用扣非净利润算 PE，避免被一次性损益误导 | 编程实践阶段 |
| 前视偏差（Look-Ahead Bias） | **回测数据纪律** | 回测时只能用调仓日之前已公开的财务数据 | 编程实践阶段 |
| 不同行业适合不同指标 | **因子构造的行业中性化** | 银行用 PB、消费用 PE、科技用 PS，行业内排序比全市场排序更合理 | 第 8 周 |
