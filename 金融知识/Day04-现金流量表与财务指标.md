...
- [ ] 知道现金流量表和利润表的区别（权责发生制 vs 收付实现制）
- [ ] 能说出三种现金流活动各是什么（经营/投资/筹资）
- [ ] 知道经营活动现金流为什么最重要
- [ ] 理解"现金奶牛型"（+/-/-）的含义
- [ ] 知道自由现金流的公式（FCF = CFO - CapEx）
- [ ] 理解 FCF Yield 和盈利质量指标在因子投资中的作用
- [ ] 能识别现金流量表的几个主要警告信号
- [ ] 能用茅台的数据说明"现金奶牛型"公司的特征
...
| 利润表的**净利润** → 现金流量表的起点（间接法） | 从利润出发，调整为现金 |
...
| 分红的具体流程（股权登记日、除息日） | 知道"公司会定期分红"就行 | 编程实践阶段会涉及 |
| 各行业详细的财务指标参考值 | 知道不同行业差异大就行 | 编程实践阶段用数据探索 |
...
|-----------|---------------|---------|---------|
| 经营现金流（CFO） | **盈利质量 → 质量因子（Quality）** | CFO / 净利润比值衡量利润含金量，比值越高盈利质量越好 | 第 10 周 |
| 自由现金流（FCF） | **FCF Yield → 价值因子（Value）替代指标** | FCF Yield = FCF / 市值，比 PE 更难被会计操纵 | 第 8 周 |
| 杜邦分解（净利率 × 周转率 × 杠杆） | **拆解 ROE → 质量因子精细化** | 区分高利润率驱动 vs 高杠杆驱动的 ROE，过滤"虚高"质量 | 第 10 周 |
| 股息率 | **红利因子（Dividend）** | 买入高股息率组、卖出低股息率组；中证红利指数即此逻辑 | 第 8 周 |
| PS（市销率） | **辅助估值指标** | PE/PB 失效时（亏损或轻资产公司）用 PS 兜底 | 编程实践阶段 |
| 现金流组合（+/-/-等） | **选股过滤条件** | "现金奶牛型"公司是因子投资青睐的标的池 | 编程实践阶段 |
| 质量因子 vs 价值因子的矛盾 | **多因子策略的核心权衡** | 高质量公司通常估值贵，多因子策略在两者间寻找平衡 | 第 10 周 |
