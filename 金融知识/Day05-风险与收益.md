...
>
> 编程实践阶段你会经常写 `np.log(price / price.shift(1))` 来计算日对数收益率——到时候就明白为什么今天要学这个了。
...
> 一年大约有 250 个交易日。用对数收益率，把 250 天的日收益率加起来就是年收益率。用简单收益率，要做 250 次乘法。当你在编程实践阶段处理几千只股票、每只几千天数据时，这个差别会非常明显。
...
其中 250 是一年的大约交易天数，sqrt(250) ≈ 15.81。

```
日波动率 1.85% → 年化波动率 = 1.85% × 15.81 ≈ 29.3%
```

> **为什么乘以 sqrt(250) 而不是 250？** 这涉及统计学中的一个性质：独立随机变量之和的方差等于各自方差之和。方差可以直接相加（250 天 → 乘 250），但标准差是方差的平方根，所以要乘 sqrt(250)。
>
> 你不需要证明这个公式，记住就行。编程实践时会用到：`annual_vol = daily_returns.std() * np.sqrt(250)`。
...
| 概念 | 今天理解到什么程度 | 什么时候深入 |
|------|------------------|------------|
| 对数收益率的数学推导 | 知道 ln 这个运算、知道可加性就行 | 数学复习阶段 |
| 标准差的样本修正（n-1） | 知道标准差衡量"偏离程度"就行 | 数学复习阶段 |
| VaR 的其他计算方法（历史模拟法、蒙特卡洛） | 知道定义和参数法公式就行 | 不涉及（太深了） |
| 条件 VaR（CVaR / Expected Shortfall） | 知道 VaR 有尾部风险的局限就行 | 不涉及 |
| Sortino 比率 | 知道它是夏普的改进版就行 | 编程实践阶段 |
| 正态分布的详细性质 | 知道"钟形曲线"和 68-95-99.7 法则就行 | 数学复习阶段 |
...
|-----------|---------------|---------|---------|
| 对数收益率 | **因子收益率计算基础** | 计算因子日收益率时用 `ln(P_t / P_{t-1})`，可加性让时间序列分析成为可能 | 编程实践阶段 |
| 波动率（标准差） | **低波动因子（Low Vol）** | 低波动率股票长期收益不输高波动率股票——"低波动异象" | Day 9-10 |
| VaR | **策略风控** | 回测因子策略时用 VaR 评估风险敞口，设定止损线 | 编程实践阶段 |
| 风险溢价 | **因子存在的理论基础** | 每个因子的超额收益可能是某种风险的补偿——风险溢价解释 | Day 8-10 |
| 夏普比率 | **策略评估的核心指标** | 比较不同因子策略好坏时，夏普比率是第一个要看的数字 | 编程实践阶段 |
| 最大回撤 | **策略可行性判断** | 即使夏普高，回撤太大的策略也不可执行——心理承受力是真实约束 | 编程实践阶段 |
