...
- 如果你觉得"稳拿 3 万就够了，我不想赌"——你的确定性等价 = 3 万（低于期望值 5 万）
...
> **这个公式在说什么？** 你的"满意度"= 期望收益（好的部分）减去一个"风险惩罚"（坏的部分）。风险越大（σ² 越大），惩罚越重；你越怕风险（A 越大），惩罚也越重。

#### 3.2 用具体数字感受

假设你在评估两个投资方案：

| 方案 | 期望收益率 E(r) | 波动率 σ | 方差 σ² |
|------|:-------------:|:-------:|:------:|
...
| A < 0 | 风险偏好 | 喜欢冒险（极少见） |
| A = 0 | 风险中性 | 只看期望收益 |
| A = 1-3 | 轻度风险厌恶 | 愿意承担较多风险换取高收益 |
| A = 3-5 | 中度风险厌恶 | 多数普通投资者 |
...
```mermaid
graph LR
    A["风险厌恶程度高（A大）<br>曲线很陡<br>要求大量收益补偿风险"] --> C["结论：A 越大<br>无差异曲线越陡"]
    B["风险厌恶程度低（A小）<br>曲线较平<br>不需要太多收益补偿"] --> C
```
...
- [ ] 理解分散化的直觉——雨伞和冰淇淋的例子
- [ ] 知道风险厌恶的含义：同等期望收益下，选风险低的
- [ ] 知道确定性等价的概念
- [ ] 能写出效用函数公式：U = E(r) - 0.5 × A × σ²
- [ ] 能用效用函数比较两个投资方案
- [ ] 知道 A 越大越怕风险，无差异曲线越陡
- [ ] 理解"没有客观上更好的投资，只有对你来说更好的投资"
...
| 风险资产（沪深 300 指数基金） | E(r_p) = 10% | σ_p = 20% |
...
- [ ] 能写出组合收益率公式：E(r_c) = R_f + y × [E(r_p) - R_f]
- [ ] 能写出组合波动率公式：σ_c = y × σ_p
- [ ] 知道资本配置线（CAL）是什么——风险收益图上的一条直线
- [ ] 知道 CAL 的斜率 = 夏普比率
- [ ] 知道最优配比公式：y* = (E(r_p) - R_f) / (A × σ_p²)
- [ ] 理解"风险厌恶程度高 → 少配风险资产"
...
| 概念 | 今天理解到什么程度 | 什么时候深入 |
|------|------------------|------------|
| 效用函数的公理化推导 | 知道公式 U = E(r) - 0.5Aσ² 会用就行 | 不涉及 |
| 无差异曲线的精确绘制 | 知道"向右上倾斜、A 大则陡"就行 | 不涉及 |
| 最优配比公式的微积分推导 | 记住结论公式就行 | 不涉及 |
| 保证金账户、融资融券的具体规则 | 知道"可以借钱买股票"就行 | 编程实践阶段 |
...
| 效用函数 | **仓位管理工具** | 根据自身风险厌恶系数 A，确定因子策略应该配多少仓位 | 编程实践阶段 |
| 资本配置线（CAL） | **策略评估的可视化** | 夏普比率决定 CAL 斜率，高夏普因子策略让 CAL 更陡 → 更优的投资选择 | 编程实践阶段 |
| 最优配置公式 | **因子策略的仓位决策** | y* = 风险溢价 / (A × σ²) 直接决定"因子策略放多少钱" | 编程实践阶段 |
| 杠杆 | **高夏普策略的收益放大** | 机构用适度杠杆放大低波动高夏普因子策略，但需严格风控 | 编程实践阶段 |
