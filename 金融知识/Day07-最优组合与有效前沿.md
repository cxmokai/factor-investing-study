...
> **关键发现：** 现实中几乎没有 ρ = -1 的资产对。大多数股票之间的相关系数在 +0.3 到 +0.8 之间——不完全同步，但也不完全独立。好消息是：**只要 ρ < 1，分散化就有效。** 不需要 ρ = -1 才能降低风险。
...
**组合期望收益率 = w_A × E(r_A) + w_B × E(r_B)**

这部分很直觉——收益率是加权平均，没有魔法。

#### 3.2 组合波动率（关键！）

**组合方差 = w_A² × σ_A² + w_B² × σ_B² + 2 × w_A × w_B × σ_A × σ_B × ρ**
...
> **这个公式是整个投资组合理论的核心。** 看着复杂，但关键只有一点：**最后一项（2 × w_A × w_B × σ_A × σ_B × ρ）里有个 ρ。**

为什么 ρ 这么重要？

- 当 ρ = +1 时，最后一项最大 → 组合风险最大 → 分散化无效
- 当 ρ 下降时，最后一项变小 → 组合风险降低 → 分散化开始起作用
- 当 ρ = -1 时，最后一项最小（为负） → 组合风险最小 → 分散化效果最大
...
> **核心发现：** 组合收益率始终是 10%（不受 ρ 影响），但组合波动率从 20% 降到 5%！**收益不变，风险大幅降低——这就是分散化的"免费午餐"。**

> **"为什么说分散化是投资中唯一的免费午餐？"** 这句话来自诺贝尔奖得主 Markowitz。通常在投资中，降低风险必须牺牲收益（Day 5 讲过的风险-收益权衡）。但分散化是例外——它在不降低预期收益的情况下降低风险。前提是资产之间不是完全正相关的。

#### 3.4 相关系数为什么不能改变收益率

有读者可能会问：**为什么 ρ 只影响风险不影响收益？**

因为收益率的期望值是线性运算（加权平均），而风险（方差）是非线性运算（涉及交叉项）。这个交叉项里包含了 ρ，所以 ρ 能影响风险。但 ρ 在收益率的加权平均中根本不出现。
...
> 用数学语言说：所有股票和市场的相关系数 ρ > 0。只要 ρ 不等于零，组合中就剩余一部分无法消除的风险——这就是系统性风险。
>
...
- [ ] 知道相关系数 ρ 的范围是 -1 到 +1
- [ ] 能写出两资产组合的波动率公式
- [ ] 理解 ρ 越低，组合风险越低——分散化效果越好
- [ ] 知道现实中大部分股票之间 ρ 在 +0.3 到 +0.8 之间
...
- 当 ρ = +1 时：所有组合在一条直线上（A 到 B 的连线）
- 当 ρ < +1 时：线向左弯曲——**弯曲的部分就是分散化带来的风险降低**
- ρ 越小，弯曲越明显
...
| 局限 | 说明 | 因子投资怎么处理 |
|------|------|----------------|
| **需要估计大量参数** | N 只股票需要估计 N 个收益率、N 个波动率、N×(N-1)/2 个相关系数。1000 只股票 = 近 50 万个相关系数 | 用因子模型降维——不直接估计股票间的相关系数，而是通过因子间接推算 |
...
组合波动率的完整计算涉及所有两两协方差（计算比较多，编程实践中用 Python 秒算），但大致结果约为 **18-19%**。
...
| 概念 | 今天理解到什么程度 | 什么时候深入 |
|------|------------------|------------|
| 协方差的精确计算公式 | 知道"衡量两资产一起动的程度"就行 | 数学复习阶段 |
| N 资产组合方差的矩阵形式 | 知道两资产公式就行 | 数学复习阶段（线性代数） |
| 有效前沿的数值求解方法（二次规划） | 知道概念就行，编程时调库 | 编程实践阶段 |
| 最小方差组合的解析解 | 知道它是前沿最左端就行 | 不涉及 |
| Black-Litterman 模型 | 不需要知道 | 不涉及 |
| 缩减估计（Shrinkage Estimation） | 知道"参数估计是个问题"就行 | 编程实践阶段 |
| 投资组合再平衡策略 | 今天不涉及 | 编程实践阶段 |
...
| 组合波动率公式 | **因子组合风险计算** | 回测多因子策略时用此公式（矩阵版）计算组合的预期波动率 | 编程实践阶段 |
| 系统性 vs 非系统性风险 | **因子策略的风险分解** | 因子策略自动分散了非系统性风险，剩余的是系统性风险和因子风险 | Day 8 |
| 有效前沿 | **多因子策略的优化目标** | 寻找最优的因子权重组合（价值多少、动量多少）就是在因子空间里找有效前沿 | 编程实践阶段 |
| 切线组合 / 最优风险组合 | **因子投资的终极目标** | 构建夏普比率最高的多因子组合，使其成为比市场指数更优的"切线组合" | 编程实践阶段 |
| 分离定理 | **仓位管理原则** | 先找最优因子组合（第1步），再根据风险偏好定仓位（第2步）——两步分离 | 编程实践阶段 |
...
> CAPM 是理解因子投资的基石。Day 8-10 是整个金融知识部分的核心中的核心。
