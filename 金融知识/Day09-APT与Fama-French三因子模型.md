...
Day 8 学了 CAPM——资产收益由一个因子（市场因子）决定：
...
> **今天是整个金融知识部分最核心的一天。** CAPM 是起点，但三因子模型才是因子投资的真正基石。从今天开始，你学的每一个概念都会在编程实践阶段直接用到。
...
| E(r_i) | 资产 i 的期望收益率 |
| r_f | 无风险利率 |
| β_i1, β_i2, ... | 资产 i 对第 1、2、... 个因子的**敏感度**（因子载荷） |
| λ_1, λ_2, ... | 第 1、2、... 个因子的**风险溢价**（因子溢价） |
| k | 因子的数量 |
...
```
r_i - r_f = α_i + β_i1 × F_1 + β_i2 × F_2 + ... + β_ik × F_k + ε_i

其中：
r_i - r_f  = 资产 i 的超额收益
α_i        = Alpha（模型解释不了的部分）
β_i1...β_ik = 资产 i 对各因子的载荷
F_1...F_k   = 各因子的收益率
ε_i        = 残差（随机噪音，和任何因子都无关）
```
...
**账面市值比（Book-to-Market Ratio, B/M）= 账面价值 / 市值**

- **账面价值**：公司在资产负债表上的净资产（Day 3 学过）
- **市值**：股价 × 总股数（Day 1 学过）
...
**MKT（Market Factor）= 市场组合收益率 - 无风险利率 = r_m - r_f**
...
**SMB（Small Minus Big）= 小盘股组合收益率 - 大盘股组合收益率**
...
**HML（High Minus Low）= 高 B/M 股票组合收益率 - 低 B/M 股票组合收益率**
...
| α_i | Alpha——三个因子都解释不了的部分 |
| β_MKT | 资产对市场因子的载荷（和 CAPM 的 Beta 类似） |
| β_SMB | 资产对市值因子的载荷——正值 = 偏小盘，负值 = 偏大盘 |
| β_HML | 资产对价值因子的载荷——正值 = 偏价值，负值 = 偏成长 |
| MKT, SMB, HML | 三个因子当期的收益率 |
| ε_i | 残差——随机噪音 |

> **和 CAPM 的对比：** CAPM 只有一个 β_MKT。三因子模型多了 β_SMB 和 β_HML——它不仅问"你对市场有多敏感"，还问"你像小盘股还是大盘股""你像价值股还是成长股"。
...
> **这张表是因子投资的核心工具。** 知道一个股票或组合的因子载荷，就知道它的收益来源——是靠市场涨带动的，还是靠小盘股溢价，还是靠价值股溢价。编程实践阶段，你会用 Python 对每只股票做回归，算出这三个载荷。
...
| CAPM 的异常 | 三因子模型的解释 |
|------------|--------------|
| 小盘股效应（小盘股 Alpha 为正） | 不是 Alpha，是 SMB 因子的贡献——小盘股的 β_SMB 高，所以收益本来就该高 |
| 价值股效应（价值股 Alpha 为正） | 不是 Alpha，是 HML 因子的贡献——价值股的 β_HML 高，所以收益本来就该高 |
...
加密市场的因子研究还在早期，但已有一些发现：

| 传统因子 | 在加密市场的对应 | 现状 |
|---------|:-------------:|------|
| MKT | 比特币因子（BTC 相对于无风险利率的超额收益） | 最重要的因子——多数加密资产和 BTC 高度相关 |
| SMB | 小市值代币 vs 大市值代币 | 有初步证据，但数据质量和生存偏差是问题 |
| HML | 尚无公认的对应物 | 加密资产缺乏"账面价值"概念，需要不同的估值指标 |

> **加密市场的因子研究是前沿领域。** 传统的 B/M 在加密市场没有直接对应物（代币没有资产负债表），需要构造新的估值指标。这是编程实践阶段可以探索的方向。
...
- [ ] 能写出三因子模型的公式
- [ ] 理解因子载荷的含义——β_SMB 正 = 偏小盘，β_HML 正 = 偏价值
...
| SMB/HML 的精确构造细节（双重排序、加权方法） | 知道"做多-做空"的概念就行 | 编程实践阶段（会用 Python 复现） |
| Fama-French 的原始论文细节 | 知道核心结论就行 | 选读 |
| 因子溢价的精确历史数值 | 知道大致范围就行（SMB 约 2-3%，HML 约 3-5%） | 编程实践阶段 |
| 四因子、五因子的公式 | 知道"在三因子上加因子"就行 | Day 10 |
| 因子模型的统计检验方法（GRS 检验等） | 不需要知道 | 不涉及 |
| 加密市场的因子模型细节 | 知道"是前沿领域"就行 | 编程实践阶段 |
...
| 因子载荷与因子溢价 | **因子投资的核心词汇** | 因子载荷 = 资产对因子的敏感度，因子溢价 = 承担因子风险的回报。构造策略就是选高载荷的资产 | 编程实践阶段 |
| SMB（市值因子） | **经典因子之一** | 买入小盘股、卖出大盘股。A 股小盘股效应尤其显著 | Day 10 + 编程实践阶段 |
| HML（价值因子） | **经典因子之一** | 买入低估值、卖出高估值。构造价值因子需要用到 Day 3-4 学的 PB/PE | Day 10 + 编程实践阶段 |
| 三因子模型公式 | **因子回归的模板** | 编程实践中对每只股票做三因子回归，得到因子载荷和 Alpha | 编程实践阶段 |
| 做多-做空构造法 | **因子策略的标准方法** | 几乎所有因子都用 Long-Short 方式构造，对冲市场风险 | 编程实践阶段 |
| CAPM 的 Alpha → 三因子的 Beta | **因子研究的核心逻辑** | 找到新因子 = 把别人的 Alpha 变成你的 Beta，这是因子投资学术进展的主线 | Day 10 |

> 今天标志着从"理解收益的来源"到"构造因子策略"的转折点。CAPM 告诉你收益由 Beta 决定，三因子模型告诉你 Beta 不止一个——市值和估值也是独立的收益来源。从 Day 10 开始，我们将把这些学术发现整合成一个完整的因子投资框架，为编程实践做最后的理论准备。
...
> Day 10 是金融知识部分的倒数第二天，会把 Day 1-9 的所有知识串成一条完整的线，为你构建因子投资的全景图。
