...
> **这六个因子是因子投资的"核心武器库"。** 绝大部分因子投资策略，都是从这六个因子的某种组合出发的。编程实践阶段，你会亲手构造这些因子并回测它们的表现。
...
> **这 6-7 个因子就是因子投资的"主菜"。** 400 多个因子经过筛选，真正经得起检验的就这些。编程实践阶段，你会聚焦在这些因子上，而不是去追逐最新发表的"第 401 个因子"。
...
> **因子投资的独特优势：** 它的收益来源有学术理论支撑（因子溢价），成本低于主动管理，而且完全可以用程序自动化。这正是为什么你会在编程实践阶段用 Python 来实现因子策略——它天然适合量化。
...
> **实践中两种方法都有人用。** 编程实践阶段会先用混合法（更容易实现），再尝试组合法。
...
| **编程实践**（30 天） | Python 数据分析、因子构造、回测 | 把今天学的所有理论用代码实现 |
| **石川《因子投资》Ch2-7** | 因子构造细节、回测方法论 | 边做边读，理论与实践结合 |

> **金融知识是"知道做什么"，数学是"知道怎么算"，编程是"真正去做"。** 三者缺一不可。你现在已经完成了第一个 1/3。
...
| 因子溢价的精确历史数值 | 知道大致范围就行 | 编程实践阶段（会自己算） |
| 因子构造的精确代码实现 | 知道概念流程就行 | 编程实践阶段 |
| Smart Beta 产品的具体种类 | 知道概念就行 | 不涉及 |
| 动量崩溃的精确日期和数值 | 知道"市场反转时动量会崩溃"就行 | 编程实践阶段（回测会看到） |
| 因子择时（Factor Timing） | 不需要知道 | 编程实践阶段（高阶话题） |
| 组合优化的数学细节 | 知道"找最优权重"的概念就行 | 数学复习阶段 + 编程实践阶段 |
...
|-----------|---------------|---------|---------|
| 动量因子（MOM） | **六大经典因子之一** | 做多过去赢家、做空过去输家。年化溢价最高但有崩溃风险 | 编程实践阶段 |
| 动量崩溃 | **因子风险管理** | 动量和价值的负相关性是多因子组合的核心搭配 | 编程实践阶段 |
| 盈利因子（RMW） | **六大经典因子之一** | 高盈利公司跑赢低盈利公司。用利润表数据构造 | 编程实践阶段 |
| 投资因子（CMA） | **六大经典因子之一** | 保守投资公司跑赢激进扩张公司。用资产负债表数据构造 | 编程实践阶段 |
| 因子动物园 | **因子筛选纪律** | 不追逐未经验证的新因子，聚焦经典因子 | 贯穿全程 |
| 因子投资实践流程 | **编程实践的路线图** | 选因子 → 构造组合 → 回测 → 实盘 → 归因 | 编程实践阶段 |
| 混合法 vs 组合法 | **多因子策略的两种实现** | 编程实践中先用混合法（简单），再尝试组合法 | 编程实践阶段 |
