...
> **因子投资最大的敌人不是市场，而是你自己的心理。** 有了系统性的策略，技术层面的问题不大（编程实践阶段会解决）。真正的挑战是在因子表现不好的时候**坚持执行**——不过度反应，不恐慌抛售，不追涨杀跌。
...
|-----------|---------------|---------|---------|
| 过度反应 | **价值因子（HML）** | 投资者对坏消息反应过度 → 价值股被系统性低估 → 长期跑赢 | 编程实践阶段（回测验证） |
| 反应不足 | **动量因子（MOM）** | 好消息后价格慢慢调整 → 趋势形成 → 过去的赢家继续赢 | 编程实践阶段（回测验证） |
| 损失厌恶与处置效应 | **策略执行纪律** | 因子投资者也会犯处置效应——跑输时放弃策略。需要规则化执行 | 贯穿全程 |
| 羊群效应 | **因子拥挤风险** | 太多人做同一个因子 → 因子拥挤 → 溢价下降甚至反转 | 编程实践阶段 |
| 代表性偏差 | **价值因子 + 均值回归** | 投资者外推过去的趋势 → 高估成长股、低估价值股 | 编程实践阶段 |
| 套利限制 | **因子溢价的持续性** | 套利有成本和风险 → 定价错误不会被完全纠正 → 因子溢价长期存在 | 贯穿全程 |

> 金融知识 11 天至此全部完成。你已经建立了因子投资的完整认知框架：知道"是什么"（六大因子）、"怎么做"（构造、组合、评估）、"为什么"（行为偏差 + 风险补偿 + 套利限制）。接下来进入数学复习和编程实践阶段——把这些理论变成可以运行的代码、可以赚钱的策略。
...
接下来的路：
- **数学复习（10 天）**：线性代数、概率统计、时间序列——让你能"算"
- **编程实践（30 天）**：Python 数据分析、因子构造、回测——让你能"做"
