...
> **今天是整个金融知识部分最核心的一天。** CAPM 是起点，但三因子模型才是因子投资的真正基石。从今天开始，你学的每一个概念都会在编程实战阶段直接用到。
...
> **加密市场的因子研究是前沿领域。** 传统的 B/M 在加密市场没有直接对应物（代币没有资产负债表），需要构造新的估值指标。这是编程实战阶段可以探索的方向。
...
| HML（价值因子） | **经典因子之一** | 买入低估值、卖出高估值。构造价值因子需要用到 Day 3-4 学的 PB/PE | 因子投资阶段 + 编程实战阶段 |
